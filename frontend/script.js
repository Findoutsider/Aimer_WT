...
const app = {
    ...
    async loadThemeList() {
        ...
        try {
            const themes = await App.GetThemeList();
            themes.forEach(t => {
    ...
    async onThemeChange(filename) {
        if (filename === 'default.json') {
            this.resetTheme();
            App.SaveThemeSelection("default.json");
            return;
        }
        const themeData = await App.LoadThemeContent(filename);
        if (themeData && themeData.colors) {
            this.applyTheme(themeData.colors);
            App.SaveThemeSelection(filename);
        } else {
    ...
    async init() {
        ...
        window.addEventListener('DOMContentLoaded', async () => {
            try {
                const state = (await window.InitAppState()) || {
                    game_path: "",
                    path_valid: false,
                ...
                if (state.active_theme && state.active_theme !== 'default.json') {
                    ...
                    // 加载内容
                    const themeData = await App.LoadThemeContent(state.active_theme);
                    if (themeData && themeData.colors) {
                        this.applyTheme(themeData.colors);
                    }
                }
                ...
                console.error("Startup handler failed before finishing init:", e);
                // 即使其它初始化失败，也尽量加载主题列表
                try { await this.loadThemeList(); } catch (e2) {}
            }
    ...
    async browsePath() {
        const res = await App.BrowseFolder();
        if (res) {
    ...
    async refreshVoice() {
        ...
        const mods = await App.GetVoiceList();
        app.modCache = mods;
    ...
    createModCard(mod) {
        ...
        div.innerHTML = `
            ...
            <div class="mod-info-area">
                ...
                <div class="mod-note" data-note="${safeNote}">
            ...
            <div class="mod-actions-col">
                <div class="action-icon action-btn-del" data-mod-id="${mod.id}" title="删除语音包">
                    <i class="ri-delete-bin-line"></i>
                </div>

                <div style="flex:1"></div>

                <div class="action-icon ${clsVideo}" data-link-video="${mod.link_video || ''}" title="观看介绍视频">
                    <i class="ri-play-circle-line"></i>
                </div>

                <div class="action-icon ${clsWt}" data-link-wt="${mod.link_wtlive || ''}" title="访问 WT Live 页面">
                    <i class="ri-global-line"></i>
                </div>

                <div class="action-icon ${clsBili}" data-link-bili="${mod.link_bilibili || ''}" title="访问 Bilibili">
                    <i class="ri-bilibili-line"></i>
                </div>

                <button class="${loadBtnClass}" data-mod-id="${mod.id}" title="${loadBtnTitle}">
        ...
        const deleteBtn = div.querySelector('.action-btn-del');
        if (deleteBtn) {
            deleteBtn.addEventListener('click', () => app.deleteMod(mod.id));
        }

        const videoBtn = div.querySelector('[data-link-video]');
        if (videoBtn && mod.link_video) {
            videoBtn.addEventListener('click', () => window.open(mod.link_video));
        }

        const wtBtn = div.querySelector('[data-link-wt]');
        if (wtBtn && mod.link_wtlive) {
            wtBtn.addEventListener('click', () => window.open(mod.link_wtlive));
        }

        const biliBtn = div.querySelector('[data-link-bili]');
        if (biliBtn && mod.link_bilibili) {
            biliBtn.addEventListener('click', () => window.open(mod.link_bilibili));
        }

        const loadBtn = div.querySelector(`[data-mod-id="${mod.id}"].${loadBtnClass}`);
        if (loadBtn) {
            loadBtn.addEventListener('click', () => app.openInstallModal(mod.id));
        }

        const modNote = div.querySelector('.mod-note');
        if (modNote) {
            modNote.addEventListener('mouseenter', function() {
                app.showTooltip(this, this.dataset.note);
            });
            modNote.addEventListener('mouseleave', () => app.hideTooltip());
        }
    ...
    openGitHubRepo() {
        window.open('https://github.com/Findoutsider/Aimer_WT');
    },
    ...
    async deleteMod(modId) {
        ...
        if (yes) {
            ...
            const success = await App.DeleteMod(modId);
            if (success) this.refreshVoice();
        }
    },
...
app.openInstallModal = async function (modId) {
    ...
    if (folders.length === 0) {
        ...
        folders.forEach(item => {
            ...
            // Tooltip 交互
            div.addEventListener('mouseenter', () => app.showTooltip(div, folderPath));
...
if (btnConfirmInstall) {
    btnConfirmInstall.addEventListener('click', async function () {
    ...
    try {
        ...
        const conflicts = await App.CheckInstallConflicts(app.currentModId, JSON.stringify(selection));
...
app.checkDisclaimer = async function () {
    try {
        const result = await App.CheckFirstRun();
...
app.handleShortcuts = function (e) {
    ...
    if (e.ctrlKey) {
        switch (e.key) {
            ...
            case 'r': case 'R': this.refreshVoice(); break;
...
app.init = async function () {
    ...
    window.addEventListener('DOMContentLoaded', async () => {
        ...
        // 2. 获取初始状态
        const state = (await App.InitAppState()) || {
            game_path: "",
            path_valid: false,
            active_theme: "default.json",
            theme: "Light",
...
function bindEventListeners() {
    EventsOn("ev_append_log", (htmlMsg) => {
        window.app.appendLog(htmlMsg);
    });
    EventsOn("ev_update_scan", (msg) => {
        window.app.updateSearchLog(msg);
    });
    EventsOn("search_success", (path) => {
        window.app.onSearchSuccess(path);
    });
    ...
    document.getElementById('btn-camo')?.addEventListener('click', () => app.switchTab('camo'));
    document.getElementById('btn-sight')?.addEventListener('click', () => app.switchTab('sight'));
    document.getElementById('btn-settings')?.addEventListener('click', () => app.switchTab('settings'));

    // 主题切换
    document.getElementById('btn-theme')?.addEventListener('click', () => app.toggleTheme());

    // 窗口控制按钮
    document.getElementById('btn-bili')?.addEventListener('click', () => app.openBiliSpace());
    document.getElementById('btn-github')?.addEventListener('click', () => app.openGitHubRepo());
    document.getElementById('btn-pin-title')?.addEventListener('click', () => app.togglePin());
    document.getElementById('btn-minimize')?.addEventListener('click', () => App.MinimizeApp());
    document.getElementById('btn-close')?.addEventListener('click', () => App.CloseApp());

    // 主页功能
    document.getElementById('btn-browse-path')?.addEventListener('click', () => app.browsePath());
    document.getElementById('btn-auto-search')?.addEventListener('click', () => app.autoSearch());
    document.getElementById('btn-clear-logs')?.addEventListener('click', () => app.clearLogs());

    // 语音包库页面
    document.getElementById('btn-import-zip')?.addEventListener('click', () => app.openImportModal());
    document.getElementById('btn-open-library')?.addEventListener('click', () => app.openFolder('voice'));
    document.getElementById('btn-open-pending')?.addEventListener('click', () => app.openFolder('pending'));
    document.getElementById('btn-open-game')?.addEventListener('click', () => app.openFolder('game'));
    document.getElementById('btn-refresh-voice')?.addEventListener('click', () => app.refreshVoice());
    
    // 搜索输入框
    const searchInput = document.getElementById('input-search-library');
    if (searchInput) {
        searchInput.addEventListener('input', (e) => app.filterLibrary(e.target.value));
    }

    // 设置页面
    document.getElementById('btn-settings-pending')?.addEventListener('click', () => app.openFolder('pending'));
    document.getElementById('btn-settings-library')?.addEventListener('click', () => app.openFolder('voice'));
    
    // 主题选择
    const themeSelect = document.getElementById('theme-select');
    if (themeSelect) {
        themeSelect.addEventListener('change', (e) => app.onThemeChange(e.target.value));
    }

    // 还原游戏
    document.getElementById('btn-restore-game')?.addEventListener('click', () => app.restoreGame());

    // 安装模态框
    document.getElementById('btn-cancel-install')?.addEventListener('click', () => app.closeModal('modal-install'));
    ...
    // 免责声明
    document.getElementById('btn-disclaimer-reject')?.addEventListener('click', () => app.disclaimerReject());
    document.getElementById('btn-disclaimer-agree')?.addEventListener('click', () => app.disclaimerAgree());
